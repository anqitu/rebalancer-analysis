...
data_df = pd.read_csv('data/clean.csv', parse_dates=[43])
validation_size = 0.2
seed = 2019
...
x_df, x_train, x_test, y_train, y_test = split_dataset(x_cols, target)
...
model_xgb = XGBRegressor(random_state=seed, n_jobs=-1)
model_xgb = model_xgb.fit(x_train, y_train)
evaluate_regression_model('XGB', model_xgb)

model_xgb.get_booster().feature_names = x_cols

ax = plot_importance(model_xgb.get_booster(), max_num_features=20, title='XGB Regressor Feature Importance')
fig = ax.figure
plt.tight_layout()
fig.savefig('images/xgbr_importance', figsize=(14, 10))


from pyearth import Earth
model_mars = Earth()
model_mars = model_mars.fit(x_train, y_train, xlabels=x_cols)
evaluate_regression_model('MARS', model_mars)

list(model_mars.basis_)
...
data_df
data_df.to_csv('data/clean_pred.csv', index=False)
